function rmain = main(img)
    ...
    [spl,NL] = splu(h, lmin,round(sp),total);
    [spu,NU] = splu(h, round(sp+1),lmax);
    ...
    hl=TRANSF(h(lmin+1:round(sp+1),:),pll1,pll2);

    hu=TRANSF(h(round(sp+2):lmax+1,:),plu1,plu2);

    h_trans=h;


    h_trans(lmin+1:round(sp+1),:)=hl;

    h_trans(round(sp+2):lmax+1,:)=hu;
